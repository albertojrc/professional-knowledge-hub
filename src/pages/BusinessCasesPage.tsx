...
interface BusinessCasesPageProps {
  focusId?: string | null
}
      ...
      {sortedCases.map((item) => (
        <article className={`case-playbook ${item.id === focusId ? 'focused-result-card' : ''}`} key={item.id}>
          ...
          <div className="two-column">
            <div className="manual-section"><h3>Tools</h3><BadgeList items={item.tools} tone="purple" /></div>
            <div className="manual-section"><h3>Outputs</h3><BadgeList items={item.outputs} tone="green" /></div>
          </div>
