      ...
      <article className="concept-main">
        ...
        <section className="manual-section section-lead">
          ...
          <div>
            ...
            <ul>{asset.howToUse.map((item) => <li key={item}>{item}</li>)}</ul>
          </div>
        </section>
        ...
        <section className="manual-section section-lead result-impact">
          ...
          <div>
            <h2>How is it interpreted?</h2>
            <ul>{asset.interpretation.map((item) => <li key={item}>{item}</li>)}</ul>
          </div>
        </section>
        ...
        <section className="manual-section">
          ...
          <div className="two-column">
            ...
            <div>
              <h3>Typical graphs</h3>
              <BadgeList items={asset.graphs} tone="purple" />
            </div>
          </div>
          <div className="ml-chart-grid">
            {asset.graphs.slice(0, 2).map((graph) => (
              <MlMiniChart key={graph} title={graph} type={graphToType(graph)} />
            ))}
          </div>
        </section>
        ...
        <section className="two-column">
          <div className="manual-section">
            ...
            <BadgeList items={asset.businessApplications} tone="blue" />
          </div>
          <div className="manual-section">
            <h2>Banking applications</h2>
            <BadgeList items={asset.bankingApplications} tone="amber" />
          </div>
        </section>
        ...
        <section className="manual-section result-bad section-lead">
          ...
          <div>
            ...
            <ul>{asset.commonMistakes.map((item) => <li key={item}>{item}</li>)}</ul>
          </div>
        </section>
        ...
        {(asset.pythonSnippet || asset.sqlSnippet) && (
          <section className="two-column">
            {asset.pythonSnippet && (
              <div className="manual-section">
                <h2>Python</h2>
                <pre className="code-block compact">{asset.pythonSnippet}</pre>
              </div>
            )}
            {asset.sqlSnippet && (
              <div className="manual-section">
                <h2>SQL / Data preparation</h2>
                <p>{asset.sqlSnippet}</p>
              </div>
            )}
          </section>
        )}
        ...
        {related.length > 0 ? (
          <div className="related-stack">
            {related.map((item) => item && (
              <button className="related-card" key={item.id} onClick={() => onOpenAsset(item.id)} type="button">
...
function graphToType(graph: string) {
