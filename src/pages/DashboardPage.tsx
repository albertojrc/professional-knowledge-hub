...
const sourceReviewItem: NavItem = { id: 'source-review-prep', label: 'Source Review Prep', eyebrow: 'File Review', description: 'Prepare configured materials for review and source mapping.', icon: 'SR' }
...
const primaryNavigation: NavItem[] = [materialInventoryItem, courseAreaMapItem, evidenceExpansionItem, sourceCoverageItem, sourceReviewItem, globalSearchItem, knowledgeLibraryItem, studyPathsItem, learningSessionItem, businessOsItem, professionalScenariosItem, decisionPlaybooksItem, ...navItems.filter((item) => ['data-science', 'banking-finance', 'credit-risk', 'output-atlas', 'model-library', 'business-cases', 'knowledge-map'].includes(item.id))]
...
export function DashboardPage({ onNavigate, onOpenAsset, assetProgress, pathPrefs }: DashboardPageProps) {
  ...
  return (
    <section className="page-stack">
      <div className="hero-panel personal-dashboard-hero"><div><span className="eyebrow">Professional Knowledge Operating System</span><h1>Your personal study command center for business, data science and banking.</h1><p>Sprint 2 now prepares source review, coverage QA and evidence upgrades before class-source claims become official.</p><div className="badge-list"><button className="primary-button" onClick={() => onNavigate('source-review-prep')} type="button">Open Source Review Prep</button><button className="primary-button" onClick={() => onNavigate('source-coverage-qa')} type="button">Open Source Coverage QA</button><button className="primary-button" onClick={() => onNavigate('evidence-expansion')} type="button">Open Evidence Expansion</button></div></div><div className="study-score-card"><span className="eyebrow">Completion Rate</span><strong>{completionRate}%</strong><div className="study-score-bar"><i style={{ width: `${completionRate}%` }} /></div><p>{completedCount} reviewed or mastered out of {studyData.total} assets.</p></div></div>
      ...
      <section className="manual-panel"><span className="eyebrow">Sprint 2 Flow</span><h2>From material review to professional knowledge</h2><KnowledgeChain nodes={['Source Review Prep', 'Material Inventory', 'Course Area Mapping', 'Evidence Expansion', 'Source Coverage QA', 'Asset / Output / Formula Links', 'Business Use']} /><p>Every class document should eventually be traceable to concepts, outputs, formulas, models, cases and professional decisions.</p></section>
