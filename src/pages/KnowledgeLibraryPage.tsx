      ...
      <section className="manual-panel library-filter-panel">
        <div className="library-filter-header">
          <div>
            ...
            <h2>{filteredAssets.length} of {knowledgeAssetRegistry.length} assets visible</h2>
          </div>
        ...
        <div className="active-filter-row">
          {[area, type, difficulty, category].filter((item) => item !== allValue).map((item) => <span className="badge badge-blue" key={item}>{item}</span>)}
          {query && <span className="badge badge-purple">Search: {query}</span>}
          {!query && [area, type, difficulty, category].every((item) => item === allValue) && <span className="badge badge-green">Showing full library</span>}
        </div>
      </section>
      ...
      <section className="manual-panel">
        ...
        {filteredAssets.length > 0 ? (
          ...
          <div className="empty-library-state">
            ...
            <button className="primary-button" onClick={clearFilters} type="button">Reset library</button>
          </div>
        )}
      </section>
    </section>
