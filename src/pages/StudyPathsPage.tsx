  ...
  return (
    <section className="page-stack">
      <div className="hero-panel study-path-hero">
        <div><span className="eyebrow">Sprint 1.11 · Path Actions</span><h1>Professional learning paths built from your Knowledge Assets.</h1><p>Choose a role-based path, set your current focus and pin paths that matter most.</p></div>
...
function PathOverview({ path, progress, onOpenAsset, pathPrefs }: { path: StudyPath; progress: ReturnType<typeof getStudyPathProgress>; onOpenAsset: (assetId: string) => void; pathPrefs: PathPrefsApi }) {
  return (
    <div className="path-overview-stack">
      ...
      <section className="manual-panel"><span className="eyebrow">Milestones</span><h2>How this path is structured</h2><div className="path-milestone-grid">{path.milestones.map((milestone, index) => { const milestoneAssets = getPathAssets({ ...path, assetIds: milestone.assetIds }); return <article className="path-milestone-card" key={milestone.id}><span>{index + 1}</span><h3>{milestone.title}</h3><p>{milestone.description}</p><div className="path-asset-mini-list">{milestoneAssets.map((asset) => <button key={asset.id} onClick={() => onOpenAsset(asset.id)} type="button"><strong>{asset.title}</strong><small>{asset.area} · {asset.category}</small></button>)}</div></article> })}</div></section>
      <section className="manual-panel"><span className="eyebrow">Full Asset Sequence</span><h2>Recommended learning order</h2><div className="path-asset-sequence">{progress.assets.map((asset, index) => <button className="path-asset-row" key={asset.id} onClick={() => onOpenAsset(asset.id)} type="button"><span>{index + 1}</span><div><strong>{asset.title}</strong><small>{asset.area} · {asset.category}</small></div><b>{asset.difficulty}</b></button>)}</div></section>
    </div>
  )
}
