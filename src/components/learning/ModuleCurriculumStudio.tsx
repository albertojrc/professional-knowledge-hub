...
export function ModuleCurriculumStudio({ curriculum, focusId, onOpenAsset }: ModuleCurriculumStudioProps) {
  ...
  return (
    <section className="concept-learning-page curriculum-studio-page">
      <div className="lesson-toolbar curriculum-toolbar">
      ...
      <div className="concept-two-pane curriculum-grid">
        <article className="concept-main lesson-paper curriculum-main">
          <ModuleHeader curriculum={curriculum} />
          <CurriculumSequence submodules={curriculum.submodules} selectedId={selected.id} />
        ...
        <aside className="concept-output-panel curriculum-side">
...
function ModuleHeader({ curriculum }: { curriculum: ModuleCurriculum }) {
  ...
  return <header className="concept-hero lesson-hero curriculum-hero"><span className="asset-icon large">{icon}</span><div><span className="eyebrow">Module Curriculum</span><h1>{curriculum.title}</h1><p>{curriculum.description}</p><div className="curriculum-rule-pill">Module → Submodule → Topic → Deep Detail View</div></div></header>
}

function CurriculumSequence({ submodules, selectedId }: { submodules: ModuleCurriculumSubmodule[]; selectedId: string }) {
  return <section className="curriculum-sequence" aria-label="Curriculum sequence">{submodules.map((submodule, index) => <div className={selectedId === submodule.id ? 'active' : ''} key={submodule.id}><span>{index + 1}</span><strong>{submodule.title.replace(/^\d+\.\s*/, '')}</strong></div>)}</section>
}
...
function SubmoduleNavigation({ submodules, selectedId, onSelect }: { submodules: ModuleCurriculumSubmodule[]; selectedId: string; onSelect: (id: string) => void }) {
  return <section className="manual-panel curriculum-panel"><div className="library-filter-header"><div><span className="eyebrow">Submodule navigation</span><h2>Choose a submodule first.</h2></div></div><div className="curriculum-submodule-grid">{submodules.map((submodule, index) => <button className={`curriculum-submodule-card ${selectedId === submodule.id ? 'selected' : ''}`} key={submodule.id} onClick={() => onSelect(submodule.id)} type="button"><span className="curriculum-number">{index + 1}</span><div><span className="eyebrow">{submodule.assetIds.length} topics · {submodule.outputs.length} outputs</span><h3>{submodule.title}</h3><p>{submodule.objective}</p></div></button>)}</div></section>
}
...
function SubmoduleDetail({ submodule, onOpenAsset }: { submodule: ModuleCurriculumSubmodule; onOpenAsset: (assetId: string) => void }) {
  ...
  return <section className="manual-panel curriculum-panel"><div className="library-filter-header"><div><span className="eyebrow">{submodule.title}</span><h2>Ordered topics</h2></div></div><div className="curriculum-topic-context"><div><h3>Theory</h3><BadgeList items={submodule.theory} tone="blue" /></div><div><h3>Outputs</h3><BadgeList items={submodule.outputs} tone="green" /></div></div><div className="curriculum-topic-grid">{assets.map((asset, index) => asset && <button className="curriculum-topic-card" key={asset.id} onClick={() => onOpenAsset(asset.id)} type="button"><span className="curriculum-number small">{index + 1}</span><div><span className="eyebrow">{asset.type} · {asset.difficulty}</span><h3>{asset.title}</h3><p>{asset.summary}</p><div className="search-result-meta"><span>{asset.category}</span><span>Open detail view</span></div></div></button>)}</div></section>
}
