  ...
  return (
    <aside className="sidebar">
      <div className="brand-panel">
        <span className="eyebrow" data-qa={sprintMarkers}>Clean Navigation</span>
        <h1>Professional Knowledge Hub</h1>
        <p>Five main modules. Each module is organized as submodules, topics and detailed study views.</p>
      </div>
      <nav className="nav-group" aria-label="Primary navigation">
        <div className="nav-title">Main Modules</div>
        {primaryNav.map((item) => (
          <button className={`nav-item ${activeView === item.id ? 'active' : ''}`} key={item.id} onClick={() => onChangeView(item.id)} type="button">
            <span className="nav-icon">{item.icon}</span>
            <span><strong>{item.label}</strong><small>{item.eyebrow}</small></span>
          </button>
        ))}
      </nav>
      <div className="sidebar-note"><strong>Flow:</strong> entra a un módulo, elige un submódulo, abre un tópico y estudia su vista de detalle. Las páginas internas siguen accesibles por búsqueda.</div>
    </aside>
  )
}
