...
interface ModuleSearchCapsulesProps { title: string; eyebrow: string; description: string; areaHints?: string[]; targetViews?: ViewId[]; keywords?: string[]; focusId?: string | null; onNavigate?: (view: ViewId, focusId?: string | null) => void; onOpenAsset?: (assetId: string) => void }
const allValue = 'All'
export function ModuleSearchCapsules({ title, eyebrow, description, areaHints = [], targetViews = [], keywords = [], focusId, onNavigate, onOpenAsset }: ModuleSearchCapsulesProps) {
  ...
  const visible = useMemo(() => { const q = query.trim().toLowerCase(); return moduleItems.filter((item) => kind === allValue || item.kind === kind).filter((item) => !q || `${item.title} ${item.kind} ${item.area} ${item.category} ${item.summary} ${item.tags.join(' ')}`.toLowerCase().includes(q)).sort((a, b) => (a.id === focusId ? -1 : b.id === focusId ? 1 : a.title.localeCompare(b.title))) }, [focusId, kind, moduleItems, query])
  ...
  const canOpen = Boolean(onNavigate || onOpenAsset)
...
function ModuleCapsuleDetail({ item, canOpen, onOpen }: { item: SearchResultItem; canOpen: boolean; onOpen: () => void }) { return <article className="module-capsule-detail"><div className="command-detail-top"><span className="eyebrow">{item.kind}</span><span className="command-status-pill">{item.area}</span></div><h2>{item.title}</h2><p>{item.summary}</p><section className="lesson-block"><div className="lesson-block-title"><span>1</span><h3>Where this belongs</h3></div><BadgeList items={[item.area, item.category, item.kind]} tone="blue" /></section><section className="lesson-block"><div className="lesson-block-title"><span>2</span><h3>Search signals</h3></div><BadgeList items={item.tags.slice(0, 18)} tone="purple" /></section><section className="lesson-block insight-block"><div className="lesson-block-title"><span>→</span><h3>Route</h3></div><p>This capsule uses the same index as search.</p>{canOpen && <button className="primary-button" onClick={onOpen} type="button">Open</button>}</section></article> }
